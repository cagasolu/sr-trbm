...
#
#   Each run binds a single process to a dedicated NVIDIA GPU using CUDA.
#   Random seeds must be set per run to guarantee reproducibility.
#
#   Average runtime per seed: ~2.5 minutes on modern RTX-class GPUs.
#   Sustained GPU usage may occur during AIS and Gibbs sampling.
#   Thermal monitoring (e.g., nvidia-smi) is strongly recommended due to the high likelihood of increased temperatures.
#
#   For multi-seed experiments, execute runs sequentially to avoid GPU oversubscription.
#   Full license text stays provided in the root LICENSE file.
...
def load_mnist(device_exact):
    ...
    return torch.tensor(X_prime, device=device_exact)
...
class HybridThermodynamicRBM:
    ...
    def temperature(self):
        if self.fixed_temperature is not None:
            return torch.tensor(self.fixed_temperature, device=self.device)

        micro = torch.exp(self.log_temperature)
        macro = self.energy_temp_scale * self.energy_avg
        T = micro + macro

        return torch.clamp(T.detach(), min=1e-6, max=1e3)
    ...
    def generate_ensemble_samples(
            ...
            steps=6000,
            temp_noise=0.05,
            hidden_refine_steps=2,
            final_cool_fraction=0.15
    ):
        """
        ...
        1) Early iterations introduce stochastic temperature perturbations.
           This increases mode exploration and reduces the risk of local trapping.

        2) The final portion of the trajectory removes temperature noise,
           enforcing deterministic sharpening around low-energy configurations.

        3) Multiple hidden refinements are applied before each visible update.
           This stabilizes the latent representation and improves structural coherence
           of generated samples.

        4) Persistent initialization is used when available.
           This reduces burn-in time and keeps chains inside the learned energy manifold.

        The stationary distribution of the RBM is preserved.
        Only the convergence dynamics of the sampler are modified.
        """
    ...
    def save_ensemble_samples(
        ...
        Image.fromarray(nd_arr).save(filename)
    ...
    def ensemble_diagnostics(
        ...
        for t in range(steps):
            ...
            if t % thinning == 0:
                Fv = self.free_energy(v, T)  # shape: (n_chains, n_visible)
    ...
    @torch.no_grad()
    def ais_log_partition(self, n_runs=1000, n_intermediate=2000):
        ...
        log_w = log_weights - torch.max(log_weights)
...
@torch.no_grad()
def sample_quality_metrics(model, real_data, n_samples=10000):
    ...
    return {
        "pixel_entropy": pixel_entropy.item(),
        "diversity": diversity,
        "mean_l2": mean_l2
    }
...
def worker(Gpu_Id, seed_round, consequences):
    ...
    last_model = HybridThermodynamicRBM(
        n_visible=784,
        n_hidden=512,
        device_type=device_warm,
        fixed_temperature=None
    )

    last_model.train(train_data)
    ...
    quality = sample_quality_metrics(last_model, train_data)

    print("Pixel entropy:", quality["pixel_entropy"])

    print("Diversity:", quality["diversity"])

    print("Mean L2:", quality["mean_l2"])
